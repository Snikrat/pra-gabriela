...
/* =========================================================
   DELETE RULES
========================================================= */
const DELETE_WINDOW_MS = 60 * 60 * 1000; const NOTE_ACTIVE_WINDOW_MS = 24 * 60 * 60 * 1000; 
function onlyExpiredNotes(notes) {
  const now = Date.now();
...
function ensureAppVersionBadge() {
...
// mostra a versão bem sutil no canto
try { ensureAppVersionBadge(); } catch {}
...
/* =========================================================
   MODAL SCROLL LOCK (mobile)
========================================================= */
function setModalOpen(on) {
  document.body.classList.toggle("modalOpen", !!on);
}

// whenever you show/hide a modal, keep background locked on mobile
function syncModalLock() {
  const anyOpen =
    jarModal?.classList.contains("show") ||
    dayModal?.classList.contains("show") ||
    createModal?.classList.contains("show") ||
    confirmModal?.classList.contains("show") ||
    _focusOverlay?.classList.contains("show");
...
function openNoteFocus(note, { bg = null, fromEl = null } = {}) {
  ...
  if (actionsEl) {
    ...
    if (canDeleteNote(note, user)) {
      ...
      del.addEventListener("click", async (ev) => {
        ...
        try {
          await deletePostItFromDay(dayKey, note);
          overlay._close?.();
          renderToday();
        } catch (e) {
          console.error(e);
  ...
  // replies (mini thread)
  const dayKey = (archiveModeKey || note?._dayKey || todayCache?.key || dayKeySP());
...
function dayKeyAddDays(key, deltaDays) {
  const [y, m, d] = String(key || "").split("-").map(Number);
  if (!y || !m || !d) return dayKeySP();

  const noon = zonedToUtcEpochSP({
    year: y,
    month: m,
    day: d,
    hour: 12,
...
async function refreshActiveNotes() {
  ...
  const [todayData, yData] = await Promise.all([
    ...
    yesterdayKey === todayKey ? Promise.resolve(normalizeDayData(null, yesterdayKey)) : fetchDayNormalized(yesterdayKey),
  ]);
  ...
  activeNotesCache = out;
  activeNotesByDay = byDay;
}
...
async function markRevealedForUser(dayKey, noteId) {
...
========================================================= */
async function deletePostItFromDay(dayKey, note) {
  const u = auth.currentUser;
...
function renderToday() {
  ...
  const user = auth.currentUser;
  const notes = (Array.isArray(activeNotesCache) ? activeNotesCache : [])
  .slice()
  ...
  notes.forEach((n, i) => {
    ...
    const dayData = activeNotesByDay[n?._dayKey] || todayCache;
    ...
    if (btn) {
      btn.addEventListener("click", async (ev) => {
        ...
        try {
          await deletePostItFromDay(n?._dayKey || todayCache?.key || dayKeySP(), n);
          renderToday();
        } catch (e) {
          console.error(e);
          await refreshTodayFromDB();
    ...
    el.addEventListener("click", async () => {
      ...
      try {
        await markRevealedForUser(n?._dayKey || todayCache?.key || dayKeySP(), n.id);
      } catch (e) {
        console.error(e);
...
async function renderJarList({ reset = false } = {}) {
  ...
  try {
    ...
    if (needsFullScan) {
      ...
      // potinho: só entra o que já expirou (24h)
      arc = arc.map((item) => ({ ...item, notes: onlyExpiredNotes(item?.notes) })).filter((item) => (item.notes || []).length);
...
async function addPostItToToday({ title, finalText }) {
  ...
  const key = String(todayCache?.key || dayKeySP());
  const ref = sharedDayDoc(key);
...
loginBtn.addEventListener("click", async () => {
  ...
  try {
    // Android: redirect é o caminho mais confiável (evita problemas de popup/persistência).
    if (isMobileAndroid()) {
...
// garante persistência (IndexedDB é o mais estável no mobile; cai pra localStorage se precisar)
(async () => {
  try {
...
// se teve login via redirect, processa o retorno (se falhar, ainda assim o onAuthStateChanged pode logar)
