...
function renderToday() {
  ...
  if (!notes.length) {
    ...
    ghost.className = "note revealed ghost";
    ghost.style.setProperty("--rot", "-1deg");
    ghost.style.background = "rgba(255,255,255,.55)";
    ghost.innerHTML = `
      <div class="noteHead"><span>🤍</span></div>
      <h2>ainda não tem post-its hoje 🤍</h2>
      <p>mas talvez seja um bom dia
            pra deixar algo aqui...</p>
      <div class="noteFoot">
        <button class="ghostCta" type="button">➕ escrever</button>
      </div>
    `;
    ghost.querySelector(".ghostCta")?.addEventListener("click", (ev) => {
      ev.preventDefault();
      ev.stopPropagation();
      openCreateModal();
    });
...
function renderEmptyState() {
  ...
  board.innerHTML = `
    <div class="emptyCard">
      <div class="emptyTop">
        <div class="emptyBadge">🤍 post-its do dia</div>
        <div aria-hidden="true">🤍</div>
      </div>

      <div class="emptyTitle">ainda não tem post-its hoje</div>
      <p class="emptyText">mas talvez seja um bom dia\npra deixar algo aqui…</p>

      <div class="emptyActions">
        <button class="emptyPrimaryBtn" id="emptyWriteBtn" type="button">➕ escrever</button>
